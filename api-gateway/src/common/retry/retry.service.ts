...
export class RetryService {
  ...
  async executeWithRetry<T>(
    ...
    const totalTime = Date.now() - startTime;
    this.logger.error(
      `All ${config.maxRetries + 1} attemps failed in ${totalTime}ms`,
    );
  ...
  private calculateDelay(attempt: number, options: RetryOptions): number {
    let delay =
      options.baseDelay * Math.pow(options.backoffMultiplier, attempt); // calculo exponencial entre 2^1, 2^2, 2^3 e etc...
